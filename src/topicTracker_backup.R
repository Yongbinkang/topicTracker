...
#         print (paste0("year:", year, ", key:", key, ", i:", i, ", x_truth:", x_truth))
    ...
    res = update_mark_group (topics, x_names, evol_color_group, mark_colors, 
                               legend_colors, legend_color_index, mark_expand, k, mark_size=10)
    ...
    res = update_mark_group (topics, x_names, res$evol_color_group, res$mark_colors, 
                               legend_colors, legend_color_index, res$mark_expand, res$k, mark_size=8)
    legend_color_index = legend_color_index+1
    ...
    topics = unique(x[which(x$child %in% node_indices),][,1])
    res = update_mark_group (topics, x_names, res$evol_color_group, res$mark_colors, 
                               legend_colors, legend_color_index, res$mark_expand, res$k, mark_size=6)
    legend_color_index = legend_color_index+1
    ...
    res = update_mark_group(topics, x_names, res$evol_color_group, res$mark_colors, 
                               legend_colors, legend_color_index, res$mark_expand, res$k, mark_size=4)
    legend_color_index = legend_color_index+1
    ...
    res = update_mark_group(topics, x_names, res$evol_color_group, res$mark_colors, 
                               legend_colors, legend_color_index, res$mark_expand, res$k, mark_size=2)
    legend_color_index = legend_color_index+1
    ...
    # Plot the graph
#     print(layout_G)
#     print(min(layout_G[,1]))
#     print(max(layout_G[,1]))
    
    out_image_fname = file.path(out_image_path)
    #png(file.path(out_image_fname, units='mm', heigh=400, width=600, res=300)
...
#          edge.curved=.3,
    ...
    legend("bottomright", 
           ...
           #inset=.02, legend=c("< 20%","< 40%", "< 60%", "< 80%", "< 100%"),
           #col=c("gray","darkgreen", "orange", "red", "darkviolet"), cex=0.8,
#            inset=.02, legend=c("20% < 40%", "< 60%", "< 80%", "< 100%"),
#            col=c("gray", "orange", "red", "darkviolet"), cex=0.7,
           #inset=c(0,.2), 
           #inset=.02, 
...
#     print(print_labs)    
    ...
    # Define the ticks of y-axis (topic weight)
#     y_no_ticks = 5
#     ytick <- seq(-1,1,0.5)
#     y_labs = c()
#     for (i in 1:(y_no_ticks)) { 
#         y_labs[i]=(ytick[i]+1)/(2)
#     }
...
#     print(paste0("y_min:", y_min, ", y_max:", y_max))
#     print(paste0("y_labs:", y_labs))
#     print(paste0("ytick:", ytick))
          
    axis(side=2, at=ytick, labels=y_labs, cex.axis=0.8)
    #textplot(layout_G[,1], layout_G[,2], x_names, cex=0.7)
    # Save the plot into an image file.
    
    options(repr.plot.width=6.5)
    par(mar=c(5.1, 4.1, 6, 7.1), xpd=TRUE)
    dev.print(pdf, out_image_fname)
    #dev.copy(device = pdf, filename = out_image_fname, width = 1000, height = 500) 
    #dev.off()
